...
class Pet:
    ...
    def feed(self):  # процент голодування
        if self.eat > 100:
            self.eat = 100
        elif self.eat <= 0:
            print('Ви забули погодувати пета. Він помер')
            print('Тут ваша статистика пета')
            print(self.check_statuts())  # универсальна программа
            delete_data_database()
            exit('Гра закінчена')

        print('Ви підняли + 35% голод')
        self.eat += 35
        self.hygiena += 25

        print(f'Зараз у вас {self.eat} процентiв')

    def play(self):
        if self.mood > 100:
            self.mood = 100
        elif self.mood <= 0:
            print('Увага у ваш низький рівень щастя.', self.mood)

        print('Ви підняли +15% щасті')
        self.mood += 15

        print(f'Зараз у вас {self.mood} процентiв')

    def hygiena(self):
        if self.hygiena > 100:
            self.hygiena = 100
        elif self.hygiena <= 0:
            print('Увага у ваш низький рівень гігієна.', self.hygiena)

        print('Ви підняли +15% гігієна')
        self.hygiena += 15

        print(f'Зараз у вас {self.hygiena} процентiв')

    def sleeping(self, hours):
        if self.sleep > 100:
            self.sleep = 100
        elif self.sleep <= 0:
            print('У вас зараз пет втомлений. Рекомендуємо поспати')
            print('Тут ваша статистика пета\n', self.check_statuts())  # универсальна программа
        a = self.sleep = hours * 4
        print(f'Ви підняли +{a}% спати')

        print(f'Зараз у вас {self.sleep} процентiв')

    def check_statuts(self):
        ...
        print(textwrap.dedent(dedent_string))  # <- даний метод прибирає відступів у багаторядковому рядку
    ...
    def event_1(self):
        print('У вас потрапив івент за назвою вгадай число від Бота. Число від 0 до 100')
        print('Якщо ви вгадайте число від + 20 балів до вас на кишеню. У вас 3 спроби')
        count = 0
        a = random.randint(0, 100)
        while True:
            try:
                count += 1
                s = int(input("Бот загадав число спробуй вгадати число"))
                if count == 3:
                    print('У вас спроби закiнчились')
                    break
                elif s == a:
                    print('Ви вгадали')
                    self.scores += 20
                    ...
                    print('Неправильно введіть інше число')
            except ValueError:
                print('Помилка: ви повинні ввести ціле число')
